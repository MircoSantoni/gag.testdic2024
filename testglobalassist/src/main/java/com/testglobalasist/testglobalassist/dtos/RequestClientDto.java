...
public record RequestClientDto(
    ...
    @NotNull(message = "La dirección IP es requerida para crear un cliente")
    @Pattern(regexp = "^(?:[0-9]{1,3}\\.){3}[0-9]{1,3}$", message = "La dirección IP debe tener un formato válido (e.g., 192.168.0.1)")
    String ipAdress,
    ...
    @NotNull(message = "El nombre del país es requerido para crear un cliente")
    @Pattern(regexp = "^[A-Za-zÀ-ÿ\\s'-]{1,100}$", message = "El nombre del país solo puede contener letras, espacios, guiones y apóstrofes, y debe tener entre 1 y 100 caracteres")
    String country
) { }
